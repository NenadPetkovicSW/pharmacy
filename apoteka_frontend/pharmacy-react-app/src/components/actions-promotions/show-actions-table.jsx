...
export default function ActionsTable(){
    ...
    useEffect(() => {
      ...
      var addCols = function (num){
          for (let i = 0; i<data.length; i++) {
              ...
              +String(data[i].name)+'</span><p>Opis: '+String(data[i].description)+' </p><p>Nova cena: '+String(data[i].newPrice)+' </p>'+myButton+'</div></div>');
    ...
    useEffect(() => {
      ...
      if(clickedButton !== undefined){
        userService.addAction(clickedButton,id)
        .then(response => {

...
import scoreService from "../common/pharmacists-axios/pharmacists-score-service";
import $ from 'jquery';
import '../css/ItemCard.css'
import { useHistory } from "react-router-dom";
import { confirmAlert } from 'react-confirm-alert'; 
import { Button, Card, Container, Form, Row} from "react-bootstrap";
export default function ScorePharmacists(){
    ...
    useEffect(() => {
        ...
        pharmacistsService.getScorable(pomocna.id)
        .then(response => {
    ...
    var reply_click = function()
    {
        ...
        let pom_pharmacistsId = this.id; 

        pom_score.idPharmacist = Number(pom_pharmacistsId);
    ...
    var addCols = function (num){
        ...
        for (let i = 0; i<pharmacists.length; i++) {
            ...
            {
                if(allScores[j].idUser == pomocna_moja.id && allScores[j].idPharmacist ==  pharmacists[i].id)
                {
            ...
            let myRating = '<input type="number" value="'+scoreToShow+'" id="rating'+ String(pharmacists[i].id) +'" placeholder="your rating" step="0.5" min="1" max="5">';
            let myButtom = '<button class="button-pharmacy" id="'+pharmacists[i].id+'"position="left" type="button">Oceni</button>';
            var myPanel = $('<div class="property-card" id="Card'+String(pharmacists[i].id)+'"><div class="property-image-pharmacist"></div><div class="property-title"><span>'+String(pharmacists[i].firstName + " " + pharmacists[i].lastName)+'</span><p>Score:'+scoreToShow+'</p>'+ myRating +'<br />'+myButtom+'</div></div>');

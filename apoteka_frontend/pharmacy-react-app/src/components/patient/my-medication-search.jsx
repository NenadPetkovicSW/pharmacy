...
export default function MedicationSearch(){
    ...
    var addCols = function (num){
        for (let i = 0; i<medication.length; i++) {
            ...
            let myButtom = '<button class="button-pharmacy" id="'+medication[i].id+'"position="left" type="button">Naruči</button>';
            var myCol = $('<div class="row center-cols center-align id="row'+String(medication[i].id)+'""></div>');
            var myPanel = $('<div class="property-card" id="Card'+String(medication[i].id)+'"><div class="property-med-image"></div><div class="property-title"><span>'+String(medication[i].name)+'</span><p>Price: '+ String(medication[i].price)+' RSD</p><p>'+ pharmacy_name+'</p>'+myButtom+'</div></div>');

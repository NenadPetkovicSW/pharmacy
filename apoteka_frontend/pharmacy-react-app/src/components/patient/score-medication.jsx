...
import scoreService from "../common/medications-axios/medication-score-service";
import $ from 'jquery';
...
export default function ScoreDermatologist(){
    ...
    useEffect(() => {
        ...
        medicationsService.getScorable(pomocna.id)
        .then(response => {
    ...
    var reply_click = function()
    {
        ...
        pom_score.idMedication = Number(pom_dermatologistsId);
    ...
    var addCols = function (num){
        ...
        for (let i = 0; i<dermatologists.length; i++) {
            ...
            {
                if(allScores[j].idUser == pomocna_moja.id && allScores[j].idMedication ==  dermatologists[i].id)
                {
            ...
            var myPanel = $('<div class="property-card" id="Card'+String(dermatologists[i].id)+'"><div class="property-med-image"></div><div class="property-title"><span>'+String(dermatologists[i].name)+'</span><p>Score:'+scoreToShow+'</p>'+ myRating +'<br />'+myButtom+'</div></div>');

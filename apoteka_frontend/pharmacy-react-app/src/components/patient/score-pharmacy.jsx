...
import scoreService from "../common/pharmacies-axios/pharmacies-score-service";
import $ from 'jquery';
...
export default function ScoreDermatologist(){
    ...
    useEffect(() => {
        ...
        pharmacyService.getScorable(pomocna.id)
        .then(response => {
    ...
    var reply_click = function()
    {
        ...
        pom_score.idPharmacy = Number(pom_dermatologistsId);
    ...
    var addCols = function (num){
        ...
        for (let i = 0; i<dermatologists.length; i++) {
            ...
            {
                if(allScores[j].idUser == pomocna_moja.id && allScores[j].idPharmacy ==  dermatologists[i].id)
                {
            ...
            var myPanel = $('<div class="property-card" id="Card'+String(dermatologists[i].id)+'"><div class="property-image"></div><div class="property-title"><span>'+String(dermatologists[i].name)+'</span><p>Score:'+scoreToShow+'</p>'+ myRating +'<br />'+myButtom+'</div></div>');
